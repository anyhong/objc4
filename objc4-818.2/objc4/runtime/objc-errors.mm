...
static void _objc_crashlog(const char *message) {
    ...
    if (!oldmsg) {
        ...
        while (isUTF8Continuation(*truncmsg))
            truncmsg++;
        asprintf(&newmsg, "... %s\n%s", truncmsg, message);
    }
    else {
...
#endif
__attribute__((noreturn, cold)) void _objc_error(id self, const char *fmt, va_list ap) {
    char *buf;
...
BREAKPOINT_FUNCTION(void _objc_warn_deprecated(void));

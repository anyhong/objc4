...
static unsigned char *compress_layout(const uint8_t *bits, size_t bitmap_bits, bool weak) {
    ...
    unsigned char *const layout = (unsigned char *)calloc(bitmap_bits + 1, 1);
...
void layout_bitmap_free(layout_bitmap bits) {
    if (bits.bits)
        free(bits.bits);
}

const unsigned char *layout_string_create(layout_bitmap bits) {
...
void layout_bitmap_set_ivar(layout_bitmap bits, const char *type, size_t offset) {
 ...
 **********************************************************************/
void layout_bitmap_grow(layout_bitmap *bits, size_t newCount) {
    if (bits->bitCount >= newCount)
        return;
    bits->bitCount = newCount;
    if (bits->bitsAllocated < newCount) {
        size_t newAllocated = bits->bitsAllocated * 2;
        if (newAllocated < newCount)
            newAllocated = newCount;
 ...
 * newPos >= oldPos.
 **********************************************************************/
void layout_bitmap_slide(layout_bitmap *bits, size_t oldPos, size_t newPos) {
    size_t shift;
    size_t count;
    ...
    if (oldPos == newPos)
        return;
    if (oldPos > newPos)
        _objc_fatal("layout bitmap sliding backwards");
 ...
 **********************************************************************/
void layout_bitmap_slide_anywhere(layout_bitmap *bits, size_t oldPos, size_t newPos) {
    size_t shift;
    size_t count;
 ...
 **********************************************************************/
bool layout_bitmap_splat(layout_bitmap dst, layout_bitmap src, size_t oldSrcInstanceSize) {
    bool changed;
 ...
 * Returns YES if any of dst's bits were changed.
 **********************************************************************/
bool layout_bitmap_or(layout_bitmap dst, layout_bitmap src, const char *msg) {
    bool changed = NO;
    size_t bit;
 ...
 * Returns YES if any of dst's bits were changed.
 **********************************************************************/
bool layout_bitmap_clear(layout_bitmap dst, layout_bitmap src, const char *msg) {
    bool changed = NO;
    size_t bit;
...
void layout_bitmap_print(layout_bitmap bits) {
    size_t i;

...
static bool lockPrecedesLock(const lockorder *oldlock, const lockorder *newlock) {
...
static bool lockPrecedesLock(const void *oldlock, const void *newlock) {
...
static bool lockUnorderedWithLock(const void *oldlock, const void *newlock) {
    mutex_tt<false>::locker lock(lockOrderLock);
    ...
    if (lockPrecedesLock(oldorder->second, neworder->second) || lockPrecedesLock(neworder->second, oldorder->second)) {
        return false;
    }
...
enum class lockkind { MUTEX = 1, MONITOR = 2, RDLOCK = 3, WRLOCK = 4, RECURSIVE = 5 };
...
static void destroyLocks(void *value) {
...
static objc_lock_list &ownedLocks() {
...
static bool hasLock(objc_lock_list &locks, const void *lock, lockkind kind) {
...
static const char *sym(const void *lock) {
    ...
    if (ok && info.dli_sname && info.dli_sname[0])
        return info.dli_sname;
    else
        return "??";
}

static void setLock(objc_lock_list &locks, const void *lock, lockkind kind) {
    ...
    if (&locks != &AllLocks() && AllLocks().find(lock) != AllLocks().end()) {
        for (auto &oldlock : locks) {
            ...
            if (lockPrecedesLock(lock, oldlock.first)) {
                _objc_fatal("lock %p (%s) incorrectly acquired before %p (%s)", oldlock.first, sym(oldlock.first), lock, sym(lock));
            }
            if (!inForkPrepare() && lockUnorderedWithLock(lock, oldlock.first)) {
...
static void clearLock(objc_lock_list &locks, const void *lock, lockkind kind) {
    auto iter = locks.find(lock);
    if (iter != locks.end()) {
        auto &l = iter->second;
        if (l.k == kind) {
...
void lockdebug_remember_mutex(mutex_t *lock) {
    setLock(AllLocks(), lock, MUTEX);
}

void lockdebug_remember_recursive_mutex(recursive_mutex_t *lock) {
    setLock(AllLocks(), lock, RECURSIVE);
}

void lockdebug_remember_monitor(monitor_t *lock) {
    setLock(AllLocks(), lock, MONITOR);
}

void lockdebug_assert_all_locks_locked() {
    auto &owned = ownedLocks();

    for (const auto &l : AllLocks()) {
        if (!hasLock(owned, l.first, l.second.k)) {
            _objc_fatal("lock %p:%d is incorrectly not owned", l.first, l.second.k);
        }
    }
}

void lockdebug_assert_no_locks_locked() {
...
void lockdebug_mutex_lock(mutex_t *lock) {
...
void lockdebug_mutex_try_lock_success(mutex_t *lock) {
    auto &locks = ownedLocks();
    setLock(locks, lock, MUTEX);
}

void lockdebug_mutex_unlock(mutex_t *lock) {
    auto &locks = ownedLocks();
...
void lockdebug_mutex_assert_locked(mutex_t *lock) {
    auto &locks = ownedLocks();
...
void lockdebug_recursive_mutex_lock(recursive_mutex_t *lock) {
    auto &locks = ownedLocks();
...
void lockdebug_recursive_mutex_assert_locked(recursive_mutex_t *lock) {
    auto &locks = ownedLocks();
...
void lockdebug_recursive_mutex_assert_unlocked(recursive_mutex_t *lock) {
    auto &locks = ownedLocks();
...
void lockdebug_monitor_enter(monitor_t *lock) {
    auto &locks = ownedLocks();
...
void lockdebug_monitor_leave(monitor_t *lock) {
    auto &locks = ownedLocks();
...
void lockdebug_monitor_wait(monitor_t *lock) {
    auto &locks = ownedLocks();
...
void lockdebug_monitor_assert_locked(monitor_t *lock) {
    auto &locks = ownedLocks();
...
void lockdebug_monitor_assert_unlocked(monitor_t *lock) {
    auto &locks = ownedLocks();

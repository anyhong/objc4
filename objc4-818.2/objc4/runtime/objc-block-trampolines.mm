...
#define PREFIX "_objc_blockTrampoline"
                ...
                storage = ptrauth_auth_data(dlsym(dylib, symbol), ptrauth_key_function_pointer, 0);
                if (!storage) {
        ...
        TrampolinePointers(void *dylib)
            : impl(dylib, "Impl"), start(dylib, "Start")
#if DEBUG
              ,
              last(dylib, "Last")
#if SUPPORT_STRET
              ,
              impl_stret(dylib, "Impl_stret"), start_stret(dylib, "Start_stret"), last_stret(dylib, "Last_stret")
#endif
#endif
        {
...
public:
    ...
    uintptr_t textSegment() {
        return get()->textSegment;
    }
    uintptr_t textSegmentSize() {
        return get()->textSegmentSize;
    }

    uintptr_t dataSize() {
        return TRAMPOLINE_PAGE_SIZE;
    }

    uintptr_t impl() {
        return get()->impl.address();
    }
    uintptr_t start() {
...
struct TrampolineBlockPageGroup {
    ...
    TrampolineBlockPageGroup() : nextPageGroup(nil), nextAvailablePage(nil), nextAvailable(startIndex()), text((const void *)((uintptr_t)this + Trampolines.dataSize())) {}
    ...
    uintptr_t trampolinesForMode(int aMode) {
        ...
        return (uintptr_t)this + Trampolines.dataSize() + TRAMPOLINE_PAGE_SIZE * (1 + aMode);
    }
...
static TrampolineBlockPageGroup *_allocateTrampolinesAndData() {
    ...
    result = vm_remap(mach_task_self(), &textDest, textSourceSize, 0, VM_FLAGS_FIXED | VM_FLAGS_OVERWRITE, mach_task_self(), textSource, TRUE, &currentProtection, &maxProtection, VM_INHERIT_SHARE);
    if (result != KERN_SUCCESS) {
...
static TrampolineBlockPageGroup *getOrAllocatePageGroupWithNextAvailable() {
    runtimeLock.assertLocked();
...
static TrampolineBlockPageGroup *pageAndIndexContainingIMP(IMP anImp, uintptr_t *outIndex) {
    runtimeLock.assertLocked();
    ...
    for (TrampolineBlockPageGroup *pageGroup = HeadPageGroup; pageGroup; pageGroup = pageGroup->nextPageGroup) {
...
static ArgumentMode argumentModeForBlock(id block) {
...
void _imp_implementationWithBlock_init(void) {
    ...
    if (__progname && (strcmp(__progname, "QtWebEngineProcess") == 0 || strcmp(__progname, "Steam Helper") == 0)) {
...
BOOL imp_removeBlock(IMP anImp) {
...

    {
        ...
        while (pageGroupIterator->nextAvailablePage && pageGroupIterator->nextAvailablePage != pageGroup) {

...
void objc_exception_throw(id exception) {
    ...
    if (PrintExceptionThrow) {
        _objc_inform("EXCEPTIONS: throwing %p (%s)", (void *)exception, object_getClassName(exception));
        void *callstack[500];
...
static void default_try_enter(void *localExceptionData) {
    ...
    if (PrintExceptions)
        _objc_inform("EXCEPTIONS: entered try block %p\n", chainLink->topHandler);
}
...
static void default_throw(id value) {
    ...
    if (value == nil) {
        if (PrintExceptions)
            _objc_inform("EXCEPTIONS: objc_exception_throw with nil value\n");
        return;
    }
    if (chainLink == nil) {
        if (PrintExceptions)
            _objc_inform("EXCEPTIONS: No handler in place!\n");
        return;
    }
    if (PrintExceptions)
        _objc_inform("EXCEPTIONS: exception thrown, going to handler block %p\n", chainLink->topHandler);
    led = chainLink->topHandler;
    chainLink->topHandler = (LocalData_t *)led->pointers[1];  // pop top handler
...
static void default_try_exit(void *led) {
    ...
    if (!chainLink || led != chainLink->topHandler) {
        if (PrintExceptions)
            _objc_inform("EXCEPTIONS: *** mismatched try block exit handlers\n");
        return;
    }
    if (PrintExceptions)
        _objc_inform("EXCEPTIONS: removing try block handler %p\n", chainLink->topHandler);
    chainLink->topHandler = (LocalData_t *)chainLink->topHandler->pointers[1];  // pop top handler
}
...
static void set_default_handlers() {
    objc_exception_functions_t default_functions = {0, default_throw, default_try_enter, default_try_exit, default_extract, default_match};

    // should this always print?
    if (PrintExceptions)
        _objc_inform("EXCEPTIONS: *** Setting default (non-Foundation) exception mechanism\n");
...
OBJC_EXTERN _Unwind_Reason_Code CXX_PERSONALITY(int version, _Unwind_Action actions, uint64_t exceptionClass, struct _Unwind_Exception *exceptionObject, struct _Unwind_Context *context);
...
__attribute__((used)) void _objc_exception_noop(void) {}
__attribute__((used)) bool _objc_exception_false(void) {
    return 0;
}
// bool _objc_exception_true(void) { return 1; }
__attribute__((used)) void _objc_exception_abort1(void) {
    _objc_fatal("unexpected call into objc exception typeinfo vtable %d", 1);
}
__attribute__((used)) void _objc_exception_abort2(void) {
    _objc_fatal("unexpected call into objc exception typeinfo vtable %d", 2);
}
__attribute__((used)) void _objc_exception_abort3(void) {
    _objc_fatal("unexpected call into objc exception typeinfo vtable %d", 3);
}
__attribute__((used)) void _objc_exception_abort4(void) {
    _objc_fatal("unexpected call into objc exception typeinfo vtable %d", 4);
}
__attribute__((used)) bool _objc_exception_do_catch(struct objc_typeinfo *catch_tinfo, struct objc_typeinfo *throw_tinfo, void **throw_obj_p, unsigned outer);
}
...
static int _objc_default_exception_matcher(Class catch_cls, id exception) {
    Class cls;
    for (cls = exception->getIsa(); cls != nil; cls = cls->getSuperclass()) {
        if (cls == catch_cls)
            return 1;
    }
...
_Unwind_Reason_Code __objc_personality_v0(int version, _Unwind_Action actions, uint64_t exceptionClass, struct _Unwind_Exception *exceptionObject, struct _Unwind_Context *context) {
    bool unwinding = ((actions & _UA_CLEANUP_PHASE) || (actions & _UA_FORCE_UNWIND));

    if (PrintExceptions) {
        _objc_inform("EXCEPTIONS: %s through frame [ip=%p sp=%p] "
                     "for exception %p",
                     unwinding ? "unwinding" : "searching", (void *)(_Unwind_GetIP(context) - 1), (void *)_Unwind_GetCFA(context), exceptionObject);
    }
    ...
    return CXX_PERSONALITY(version, actions, exceptionClass, exceptionObject, context);
}
...
static void _objc_exception_destructor(void *exc_gen) {
    ...
    if (PrintExceptions) {
        _objc_inform("EXCEPTIONS: releasing completed exception %p (object %p, a %s)", exc, obj, object_getClassName(obj));
    }
...
void objc_exception_throw(id obj) {
    struct objc_exception *exc = (struct objc_exception *)__cxa_allocate_exception(sizeof(struct objc_exception));
...
bool _objc_exception_do_catch(struct objc_typeinfo *catch_tinfo, struct objc_typeinfo *throw_tinfo, void **throw_obj_p, unsigned outer UNAVAILABLE_ATTRIBUTE) {
    id exception;
    ...
    *throw_obj_p = **(void ***)throw_obj_p;
    ...
    if (!handler_cls) {
        ...
        if (PrintExceptions)
            _objc_inform("EXCEPTIONS: catch(%s)", handler_cls->nameForLogging());
        return true;
    }

    if (PrintExceptions)
        _objc_inform("EXCEPTIONS: skipping catch(%s)", handler_cls->nameForLogging());
 ...
 * and base addresses.
 **********************************************************************/
static uintptr_t read_address(uintptr_t *pp, const struct dwarf_eh_bases *bases, unsigned char encoding) {
    uintptr_t result = 0;
    uintptr_t oldp = *pp;
...
#endif
    default:
        _objc_inform("unknown DWARF EH encoding 0x%x at %p", encoding, (void *)*pp);
        break;
    }
...
static bool isObjCExceptionCatcher(uintptr_t lsda, uintptr_t ip, const struct dwarf_eh_bases *bases, struct frame_range *frame) {
...
__attribute__((noinline, noreturn)) void alt_handler_error(uintptr_t token);

static struct alt_handler_list *fetch_handler_list(bool create) {
...
void _destroyAltHandlerList(struct alt_handler_list *list) {
    if (list) {
        if (DebugAltHandlers) {
            ...
            while (*listp && *listp != list)
                listp = &(*listp)->next_DEBUGONLY;
            if (*listp)
                *listp = (*listp)->next_DEBUGONLY;
        }
...
uintptr_t objc_addExceptionHandler(objc_exception_handler fn, void *context) {
    ...
    if (list->used == list->allocated) {
        ...
        list->handlers = (struct alt_handler_data *)realloc(list->handlers, list->allocated * sizeof(list->handlers[0]));
        bzero(&list->handlers[list->used], (list->allocated - list->used) * sizeof(list->handlers[0]));
        i = list->used;
    }
    else {
        for (i = 0; i < list->allocated; i++) {
            if (list->handlers[i].frame.ip_start == 0 && list->handlers[i].frame.ip_end == 0 && list->handlers[i].frame.cfa == 0) {
                break;
            }
        }
        if (i == list->allocated) {
    ...
    if (PrintAltHandlers) {
        _objc_inform("ALT HANDLERS: installing alt handler #%lu %p(%p) on "
                     "frame [ip=%p..%p sp=%p]",
                     (unsigned long)token, data->fn, data->context, (void *)data->frame.ip_start, (void *)data->frame.ip_end, (void *)data->frame.cfa);
        if (data->frame.ips) {
            unsigned int r = 0;
            while (1) {
...
BREAKPOINT_FUNCTION(void objc_alt_handler_error(void));

__attribute__((noinline, noreturn)) void alt_handler_error(uintptr_t token) {
    _objc_inform("objc_removeExceptionHandler() called with unknown alt handler; "
        ...
        for (list = DebugLists; list; list = list->next_DEBUGONLY) {
            ...
            for (h = 0; h < list->allocated; h++) {
                ...
                if (data->debug && data->debug->token == token) {
                    ...
                    _objc_inform_now_and_on_crash("The matching objc_addExceptionHandler() was called "
    ...
    _objc_fatal("objc_removeExceptionHandler() called with unknown alt handler; "
                "this is probably a bug in multithreaded AppKit use. ");
}
        ...
        if (ip >= data->frame.ip_start && ip < data->frame.ip_end && data->frame.cfa == cfa) {
            ...
            if (PrintExceptions || PrintAltHandlers) {
                _objc_inform("EXCEPTIONS: calling alt handler %p(%p) from "
                             "frame [ip=%p..%p sp=%p]",
                             copy.fn, copy.context, (void *)copy.frame.ip_start, (void *)copy.frame.ip_end, (void *)copy.frame.cfa);
            }
            if (copy.fn)
                (*copy.fn)(nil, copy.context);
            if (copy.frame.ips)
                free(copy.frame.ips);
        }
    }

...
template <typename T> static bool method_lists_contains_any(T *mlists, T *end, SEL sels[], size_t selcount);
...
template <typename T> static constexpr T coveringMask(T n) {
...
using ClassCopyFixupHandler = void (*)(Class _Nonnull oldClass, Class _Nonnull newClass);
// Normally there's only one handler registered.
static GlobalSmallVector<ClassCopyFixupHandler, 1> classCopyFixupHandlers;

void _objc_setClassCopyFixupHandler(void (*_Nonnull newFixupHandler)(Class _Nonnull oldClass, Class _Nonnull newClass)) {
...
static Class alloc_class_for_subclass(Class supercls, size_t extraBytes) {
    ...
    swift_class_t *swcls = (swift_class_t *)((uint8_t *)bits + swiftSupercls->classAddressOffset);
 ...
 **********************************************************************/
NEVER_INLINE
static bool dataSegmentsContain(Class cls) {
    uint32_t index;
 ...
 **********************************************************************/
__attribute__((cold, noinline)) static void printReplacements(Class cls, const locstamped_category_t *cats_list, uint32_t cats_count) {
    uint32_t c;
        ...
        for (const auto &meth : *mlist) {
            ...
            for (uint32_t c2 = 0; c2 < c; c2++) {
                ...
                for (const auto &meth2 : *mlist2) {
                    ...
                    if (s == s2) {
                        logReplacedMethod(cls->nameForLogging(), s, cls->isMetaClass(), cat->name, meth2.imp(false), meth.imp(false));
                        goto complained;
                    }
                }
            }
 ...
 * Locking: runtimeLock must be held by the caller.
 **********************************************************************/
static inline void foreach_realized_class_and_subclass_2(Class top, unsigned &count, bool skip_metaclass, bool (^code)(Class) __attribute((noescape))) {
    Class cls = top;
...
static void foreach_realized_class_and_subclass(Class top, bool (^code)(Class) __attribute((noescape))) {
...
static void foreach_realized_class_and_metaclass(bool (^code)(Class) __attribute((noescape))) {
    unsigned int count = unreasonableClassCount();

    for (Class top = _firstRealizedClass; top != nil; top = top->data()->nextSiblingClass) {
        foreach_realized_class_and_subclass_2(top, count, false, code);
    }
}

// Enumerates all realized classes (ignoring metaclasses).
static void foreach_realized_class(bool (^code)(Class) __attribute((noescape))) {
    unsigned int count = unreasonableClassCount();
...
namespace scanner {
    ...
    static ALWAYS_INLINE uintptr_t swizzlingBit(SelectorBundle bundle, bool isMeta) {
        return 1UL << (2 * bundle + isMeta);
    }

    static void __attribute__((cold, noinline)) printCustom(Class cls, SelectorBundle bundle, bool inherited) {
        ...
        _objc_inform("%s: %s%s%s", SelectorBundleName[bundle], cls->nameForLogging(), cls->isMetaClass() ? " (meta)" : "", inherited ? " (inherited)" : "");
    }

    enum class Scope { Instances, Classes, Both };

    template <typename Traits, SelectorBundle Bundle, bool &ShouldPrint, Scope Domain = Scope::Both> class Mixin {
        ...
        static void setCustomRecursively(Class cls, bool inherited = false) {
        ...
        static bool isNSObjectSwizzled(bool isMeta) {
            return NSObjectSwizzledMask & swizzlingBit(Bundle, isMeta);
        }

        static void setNSObjectSwizzled(Class NSOClass, bool isMeta) {
        ...
        static void scanChangedMethodForUnknownClass(const method_t *meth) {
            Class cls;
        ...
        static void scanAddedClassImpl(Class cls, bool isMeta) {
            ...
            if (isNSObjectSwizzled(isMeta)) {
                ...
                setCustom = Traits::scanMethodLists(methods.beginCategoryMethodLists(), methods.endCategoryMethodLists(cls));
            }
                ...
                // Not NSObject.
                auto &methods = as_objc_class(cls)->data()->methods();
                setCustom = Traits::scanMethodLists(methods.beginLists(), methods.endLists());
            }
            if (slowpath(setCustom)) {
                if (ShouldPrint)
                    printCustom(cls, Bundle, inherited);
            }
            else {
    ...
    public:
        ...
        static void scanInitializedClass(Class cls, Class metacls) {
        ...
        //
        // Caller: addSubclass()
        static void scanAddedSubClass(Class subcls, Class supercls) {
        ...
        static void scanAddedMethodLists(Class cls, method_list_t **mlists, int count) {
        ...
        static void scanChangedMethod(Class cls, const method_t *meth) {
...
struct AWZScanner : scanner::Mixin<AWZScanner, AWZ, PrintCustomAWZ, scanner::Scope::Classes> {
    ...
    template <typename T> static bool scanMethodLists(T *mlists, T *end) {
        SEL sels[2] = {
            @selector(alloc),
...
struct RRScanner : scanner::Mixin<RRScanner, RR, PrintCustomRR
    ...
    static bool isInterestingSelector(SEL sel) {
        return sel == @selector(retain) || sel == @selector(release) || sel == @selector(autorelease) || sel == @selector(_tryRetain) || sel == @selector(_isDeallocating)
               || sel == @selector(retainCount) || sel == @selector(allowsWeakReference) || sel == @selector(retainWeakReference);
    }
    template <typename T> static bool scanMethodLists(T *mlists, T *end) {
        SEL sels[8] = {
            @selector(retain),
...
struct CoreScanner : scanner::Mixin<CoreScanner, Core, PrintCustomCore> {
    static bool knownClassHasDefaultImpl(Class cls, bool isMeta) {
        if (scanner::Mixin<CoreScanner, Core, PrintCustomCore>::knownClassHasDefaultImpl(cls, isMeta))
            return true;
        if ((cls->isRootClass() || cls->isRootMetaclass()) && strcmp(cls->mangledName(), "_TtCs12_SwiftObject") == 0)
            return true;
    ...
    static bool isInterestingSelector(SEL sel) {
        return sel == @selector(new) || sel == @selector(self) || sel == @selector(class) || sel == @selector(isKindOfClass:) || sel == @selector(respondsToSelector:);
    }
    template <typename T> static bool scanMethodLists(T *mlists, T *end) {
        SEL sels[5] = { @selector(new), @selector(self), @selector(class), @selector(isKindOfClass:), @selector(respondsToSelector:) };
...
public:
    void addForClass(locstamped_category_t lc, Class cls) {
        ...
        if (slowpath(PrintConnecting)) {
            _objc_inform("CLASS: found category %c%s(%s)", cls->isMetaClassMaybeUnrealized() ? '+' : '-', cls->nameForLogging(), lc.cat->name);
        }
    ...
    void attachToClass(Class cls, Class previously, int flags) {
        runtimeLock.assertLocked();
        ASSERT((flags & ATTACH_CLASS) || (flags & ATTACH_METACLASS) || (flags & ATTACH_CLASS_AND_METACLASS));
...
static void prepareMethodLists(Class cls, method_list_t **addedLists, int addedCount, bool baseMethods, bool methodsFromBundle, const char *why) {
    runtimeLock.assertLocked();
...
static void attachCategories(Class cls, const locstamped_category_t *cats_list, uint32_t cats_count, int flags) {
    ...
    if (slowpath(PrintConnecting)) {
        _objc_inform("CLASS: attaching %d categories to%s class '%s'%s", cats_count, (flags & ATTACH_EXISTING) ? " existing" : "", cls->nameForLogging(), (flags & ATTACH_METACLASS) ? " (meta)" : "");
    }
...
static void methodizeClass(Class cls, Class previously) {
    ...
    for (const auto &meth : rw->methods()) {
        if (PrintConnecting) {
            _objc_inform("METHOD %c[%s %s]", isMeta ? '+' : '-', cls->nameForLogging(), sel_getName(meth.name()));
        }
...
static char *copySwiftV1MangledName(const char *string, bool isProtocol = false) {
    ...
    if (prefixLength == 5 && memcmp(prefix, "Swift", 5) == 0) {
        asprintf(&name, "_Tt%cs%zu%.*s%s", isProtocol ? 'P' : 'C', suffixLength, (int)suffixLength, suffix, isProtocol ? "_" : "");
    }
    else {
        asprintf(&name, "_Tt%c%zu%.*s%zu%.*s%s", isProtocol ? 'P' : 'C', prefixLength, (int)prefixLength, prefix, suffixLength, (int)suffixLength, suffix, isProtocol ? "_" : "");
    }
    return name;
}
...
_Nullable Class objc_loadClassref(_Nullable Class *_Nonnull clsref) {
...
static void removeRootClass(Class cls) {
    ...
    Class *classp;
    for (classp = &_firstRealizedClass; *classp != cls; classp = &(*classp)->data()->nextSiblingClass) {
    }
...
static ALWAYS_INLINE protocol_t *remapProtocol(protocol_ref_t proto) {
    ...
    protocol_t *newproto = (protocol_t *)getProtocol(((protocol_t *)proto)->mangledName);
...
static void reconcileInstanceVariables(Class cls, Class supercls, const class_ro_t *&ro) {
    ...
    */
    ...
    if (DebugNonFragileIvars) {
        ...
        if (!strstr(clsname, "NSCF") && 0 != strncmp(clsname, "__CF", 4) && 0 != strcmp(clsname, "NSConstantString") && 0 != strcmp(clsname, "NSSimpleCString")) {
            ...
            if (PrintIvars) {
                _objc_inform("IVARS: DEBUG: forcing ivars for class '%s' "
                             ...
                             cls->nameForLogging(), (size_t)oldStart, (size_t)ro->instanceStart);
            }
...
static Class realizeClassWithoutSwift(Class cls, Class previously) {
...
#if FAST_CACHE_META
    if (isMeta)
        cls->cache.setBit(FAST_CACHE_META);
#endif
    ...
    if (PrintConnecting) {
        _objc_inform("CLASS: realizing class '%s'%s %p %p #%u %s%s", cls->nameForLogging(), isMeta ? " (meta)" : "", (void *)cls, ro, cls->classArrayIndex(), cls->isSwiftStable() ? "(swift)" : "",
...
Class _objc_realizeClassFromSwift(Class cls, void *previously) {
    if (cls) {
        ...
        _objc_fatal("Swift requested that class %p be ignored, "
                    "but libobjc does not support that.",
                    previously);
    }
}
...
void _objc_flush_caches(Class cls) {
    {
        mutex_locker_t lock(runtimeLock);
        flushCaches(cls, __func__, [](Class c) { return !c->cache.isConstantOptimizedCache(); });
        if (cls && !cls->isMetaClass() && !cls->isRootClass()) {
            flushCaches(cls->ISA(), __func__, [](Class c) { return !c->cache.isConstantOptimizedCache(); });
        }
        else {
    ...
    auto processCatlist = [&](category_t *const *catlist) {
        for (unsigned i = 0; i < count; i++) {
            ...
            if (cls->isStubClass()) {
                ...
                if (cat->instanceMethods || cat->protocols || cat->instanceProperties || cat->classMethods || cat->protocols || (hasClassProperties && cat->_classProperties)) {
    ...
    if (oldproto) {
        if (oldproto != newproto) {
            ...
            if (headerIsPreoptimized && !oldproto->isCanonical()) {
                ...
                auto cacheproto = (protocol_t *)getSharedCachePreoptimizedProtocol(newproto->mangledName);
...
#define EACH_HEADER                          \
    ...
    if (!doneOnce) {
        ...
        for (EACH_HEADER) {
            if (hi->info()->containsSwift() && hi->info()->swiftUnstableVersion() < objc_image_info::SwiftVersion3) {
    ...
    if (PrintPreopt) {
        ...
        _objc_inform("PREOPTIMIZATION: %zu selector references not "
                     "pre-optimized",
                     UnfixedSelectors);
        _objc_inform("PREOPTIMIZATION: %u/%u (%.3g%%) method lists pre-sorted", PreoptOptimizedMethodLists, PreoptTotalMethodLists,
                     PreoptTotalMethodLists ? 100.0 * PreoptOptimizedMethodLists / PreoptTotalMethodLists : 0.0);
        _objc_inform("PREOPTIMIZATION: %u/%u (%.3g%%) classes pre-registered", PreoptOptimizedClasses, PreoptTotalClasses,
                     PreoptTotalClasses ? 100.0 * PreoptOptimizedClasses / PreoptTotalClasses : 0.0);
...
bool hasLoadMethods(const headerType *mhdr) {
    size_t count;
    if (_getObjc2NonlazyClassList(mhdr, &count) && count > 0)
        return true;
    if (_getObjc2NonlazyCategoryList(mhdr, &count) && count > 0)
        return true;
    return false;
}
...
static IMP _method_setImplementation(Class cls, method_t *m, IMP imp) {
    ...
    flushCaches(cls, __func__, [sel, old](Class c) { return c->cache.shouldFlush(sel, old); });
...
void method_exchangeImplementations(Method m1, Method m2) {
    ...
    flushCaches(nil, __func__, [sel1, sel2, imp1, imp2](Class c) { return c->cache.shouldFlush(sel1, imp1) || c->cache.shouldFlush(sel2, imp2); });
...
static uint32_t getExtendedTypesIndexForMethod(protocol_t *proto, const method_t *m, bool isRequiredMethod, bool isInstanceMethod) {
    uint32_t a;
    uint32_t b;
    getExtendedTypesIndexesForMethod(proto, m, isRequiredMethod, isInstanceMethod, a, b);
    return a + b;
}
 ...
 **********************************************************************/
static void fixupProtocolMethodList(protocol_t *proto, method_list_t *mlist, bool required, bool instance) {
    runtimeLock.assertLocked();
    ...
    fixupMethodList(mlist, true /*always copy for simplicity*/, !extTypes /*sort if no extended method types*/);
    ...
    if (extTypes && !mlist->isSmallList()) {
        ...
        uint32_t prefix;
        uint32_t junk;
        getExtendedTypesIndexesForMethod(proto, &mlist->get(0), required, instance, prefix, junk);
 ...
 **********************************************************************/
static void fixupProtocol(protocol_t *proto) {
    runtimeLock.assertLocked();
 ...
 * Locking: write-locks runtimeLock.
 **********************************************************************/
static void fixupProtocolIfNeeded(protocol_t *proto) {
    runtimeLock.assertUnlocked();
    ASSERT(proto);
 ...
 * Locking: runtimeLock must be held by the caller
 **********************************************************************/
static method_t *protocol_getMethod_nolock(protocol_t *proto, SEL sel, bool isRequiredMethod, bool isInstanceMethod, bool recursive) {
    runtimeLock.assertLocked();
    ...
    method_list_t *mlist = getProtocolMethodList(proto, isRequiredMethod, isInstanceMethod);
    if (mlist) {
    ...
    if (recursive && proto->protocols) {
        ...
        for (uint32_t i = 0; i < proto->protocols->count; i++) {
            ...
            m = protocol_getMethod_nolock(realProto, sel, isRequiredMethod, isInstanceMethod, true);
            if (m)
                return m;
        }
    }
...
Method protocol_getMethod(protocol_t *proto, SEL sel, bool isRequiredMethod, bool isInstanceMethod, bool recursive) {
    ...
    mutex_locker_t lock(runtimeLock);
    return protocol_getMethod_nolock(proto, sel, isRequiredMethod, isInstanceMethod, recursive);
}
 ...
 * Locking: runtimeLock must be held by the caller
 **********************************************************************/
const char *protocol_getMethodTypeEncoding_nolock(protocol_t *proto, SEL sel, bool isRequiredMethod, bool isInstanceMethod) {
    runtimeLock.assertLocked();
    ...
    method_t *m = protocol_getMethod_nolock(proto, sel, isRequiredMethod, isInstanceMethod, false);
    if (m) {
        uint32_t i = getExtendedTypesIndexForMethod(proto, m, isRequiredMethod, isInstanceMethod);
    ...
    if (proto->protocols) {
        for (uintptr_t i = 0; i < proto->protocols->count; i++) {
            const char *enc = protocol_getMethodTypeEncoding_nolock(remapProtocol(proto->protocols->list[i]), sel, isRequiredMethod, isInstanceMethod);
            if (enc)
                return enc;
        }
    }
...
const char *_protocol_getMethodTypeEncoding(Protocol *proto_gen, SEL sel, BOOL isRequiredMethod, BOOL isInstanceMethod) {
    ...
    mutex_locker_t lock(runtimeLock);
    return protocol_getMethodTypeEncoding_nolock(proto, sel, isRequiredMethod, isInstanceMethod);
}
...
const char *protocol_t::demangledName() {
    ...
    if (!_demangledName) {
        ...
        if (!OSAtomicCompareAndSwapPtrBarrier(nil, (void *)(de ?: mangledName), (void **)&_demangledName)) {
            if (de)
                free(de);
        }
    }
...
const char *protocol_getName(Protocol *proto) {
    if (!proto)
        ...
        return newprotocol(proto)->demangledName();
}
...
struct objc_method_description protocol_getMethodDescription(Protocol *p, SEL aSel, BOOL isRequiredMethod, BOOL isInstanceMethod) {
    Method m = protocol_getMethod(newprotocol(p), aSel, isRequiredMethod, isInstanceMethod, true);
    // method_getDescription is inefficient for small methods. Don't bother
    // trying to use it, just make our own.
    if (m)
        return (struct objc_method_description){m->name(), (char *)m->types()};
    else
        return (struct objc_method_description){nil, nil};
}
...
BOOL protocol_conformsToProtocol(Protocol *self, Protocol *other) {
    mutex_locker_t lock(runtimeLock);
    return protocol_conformsToProtocol_nolock(newprotocol(self), newprotocol(other));
}
...
BOOL protocol_isEqual(Protocol *self, Protocol *other) {
    ...
    if (!protocol_conformsToProtocol(self, other))
        return NO;
    if (!protocol_conformsToProtocol(other, self))
        return NO;
...
struct objc_method_description *protocol_copyMethodDescriptionList(Protocol *p, BOOL isRequiredMethod, BOOL isInstanceMethod, unsigned int *outCount) {
    ...
    if (mlist) {
        result = (struct objc_method_description *)calloc(mlist->count + 1, sizeof(struct objc_method_description));
        for (const auto &meth : *mlist) {
 ...
 * Locking: runtimeLock must be held by the caller
 **********************************************************************/
static property_t *protocol_getProperty_nolock(protocol_t *proto, const char *name, bool isRequiredProperty, bool isInstanceProperty) {
    runtimeLock.assertLocked();
    ...
    property_list_t *plist = isInstanceProperty ? proto->instanceProperties : proto->classProperties();
    if (plist) {
        for (auto &prop : *plist) {
    ...
    if (proto->protocols) {
        ...
        for (i = 0; i < proto->protocols->count; i++) {
            ...
            property_t *prop = protocol_getProperty_nolock(p, name, isRequiredProperty, isInstanceProperty);
            if (prop)
                return prop;
        }
    }
...
objc_property_t protocol_getProperty(Protocol *p, const char *name, BOOL isRequiredProperty, BOOL isInstanceProperty) {
    ...
    mutex_locker_t lock(runtimeLock);
    return (objc_property_t)protocol_getProperty_nolock(newprotocol(p), name, isRequiredProperty, isInstanceProperty);
}
...
objc_property_t *protocol_copyPropertyList2(Protocol *proto, unsigned int *outCount, BOOL isRequiredProperty, BOOL isInstanceProperty) {
    ...
    property_list_t *plist = isInstanceProperty ? newprotocol(proto)->instanceProperties : newprotocol(proto)->classProperties();
...
void protocol_addProtocol(Protocol *proto_gen, Protocol *addition_gen) {
    ...
    if (addition->ISA() == cls) {
        _objc_inform("protocol_addProtocol: added protocol '%s' is still "
                     "under construction!",
                     addition->nameForLogging());
        return;
    }

    protocol_list_t *protolist = proto->protocols;
    if (!protolist) {
        protolist = (protocol_list_t *)calloc(1, sizeof(protocol_list_t) + sizeof(protolist->list[0]));
    }
    else {
        protolist = (protocol_list_t *)realloc(protolist, protocol_list_size(protolist) + sizeof(protolist->list[0]));
    }
...
void protocol_addMethodDescription(Protocol *proto_gen, SEL name, const char *types, BOOL isRequiredMethod, BOOL isInstanceMethod) {
    protocol_t *proto = newprotocol(proto_gen);
 ...
 * Locking: acquires runtimeLock
 **********************************************************************/
static void protocol_addProperty_nolock(property_list_t *&plist, const char *name, const objc_property_attribute_t *attrs, unsigned int count) {
    if (!plist) {
...
void protocol_addProperty(Protocol *proto_gen, const char *name, const objc_property_attribute_t *attrs, unsigned int count, BOOL isRequiredProperty, BOOL isInstanceProperty) {
    protocol_t *proto = newprotocol(proto_gen);
...
static int objc_getRealizedClassList_nolock(Class *buffer, int bufferLen) {
    int count = 0;
...
static Class *objc_copyRealizedClassList_nolock(unsigned int *outCount) {
    Class *result = nil;
    unsigned int count = 0;
 ...
 **********************************************************************/
objc_imp_cache_entry *class_copyImpCache(Class cls, int *outCount) {
...
Protocol *__unsafe_unretained *objc_copyProtocolList(unsigned int *outCount) {
    ...
    while (NXNextMapState(protocol_map, &state, (const void **)&name, (const void **)&proto)) {
        result[i++] = proto;
    }
...
#if TARGET_OS_WIN32
    const TCHAR **names = (const TCHAR **)malloc((HeaderCount + 1) * sizeof(TCHAR *));
#else
    const char **names = (const char **)malloc((HeaderCount + 1) * sizeof(char *));
#endif
 ...
 * Locking: runtimeLock must be held by the caller
 **********************************************************************/
const char **copyClassNamesForImage_nolock(header_info *hi, unsigned int *outCount) {
    runtimeLock.assertLocked();
    ASSERT(hi);

    size_t count;
    classref_t const *classlist = _getObjc2ClassList(hi, &count);
    const char **names = (const char **)malloc((count + 1) * sizeof(const char *));
...
Class *copyClassesForImage_nolock(header_info *hi, unsigned int *outCount) {
    runtimeLock.assertLocked();
    ASSERT(hi);

    size_t count;
    classref_t const *classlist = _getObjc2ClassList(hi, &count);
    Class *classes = (Class *)malloc((count + 1) * sizeof(Class));
...
#if TARGET_OS_WIN32
        if (0 == wcscmp((TCHAR *)image, hi->moduleName))
            break;
#else
        if (0 == strcmp(image, hi->fname()))
            break;
#endif
    }
...
Class *objc_copyClassesForImage(const char *image, unsigned int *outCount) {
    if (!image) {
        if (outCount)
            *outCount = 0;
        return nil;
...
const char *objc_class::demangledName(bool needsLock) {
    if (!needsLock) {
        runtimeLock.assertLocked();
    }
    ...
    if (isRealized() || isFuture()) {
        ...
        if (!OSAtomicCompareAndSwapPtrBarrier(nil, (void *)(de ?: mangled), (void **)&rwe->demangledName)) {
            if (de)
                free(de);
        }
        return rwe->demangledName;
...
template <class getNameFunc> ALWAYS_INLINE static method_t *findMethodInUnsortedMethodList(SEL sel, const method_list_t *list, const getNameFunc &getName) {
    for (auto &meth : *list) {
        if (getName(meth) == sel)
            return &meth;
    }
...
NEVER_INLINE static method_t *search_method_list(const method_list_t *mlist, SEL sel) {
 ...
 **********************************************************************/
template <typename T> static NEVER_INLINE bool method_lists_contains_any(T *mlists, T *end, SEL sels[], size_t selcount) {
    while (mlists < end) {
    ...
    if (resolved && PrintResolving) {
        if (imp) {
            _objc_inform("RESOLVE: method %c[%s %s] "
                         ...
                         cls->isMetaClass() ? '+' : '-', cls->nameForLogging(), sel_getName(sel), imp);
        }
        else {
            // Method resolver didn't add anything?
            _objc_inform("RESOLVE: +[%s resolveClassMethod:%s] returned YES"
                         ", but no new implementation of %c[%s %s] was found",
                         cls->nameForLogging(), sel_getName(sel), cls->isMetaClass() ? '+' : '-', cls->nameForLogging(), sel_getName(sel));
        }
    }
}
...
static void log_and_fill_cache(Class cls, IMP imp, SEL sel, id receiver, Class implementer) {
    ...
    if (slowpath(objcMsgLogEnabled && implementer)) {
        bool cacheIt = logMessageSend(implementer->isMetaClass(), cls->nameForLogging(), implementer->nameForLogging(), sel);
        if (!cacheIt)
            return;
    }
#endif
...
void objc_class::setInitialized() {
    ...
    if (PrintInitializing) {
        _objc_inform("INITIALIZE: thread %p: setInitialized(%s)", objc_thread_self(), cls->nameForLogging());
    }
...
void objc_class::printInstancesRequireRawIsa(bool inherited) {
    ASSERT(PrintRawIsa);
    ASSERT(instancesRequireRawIsa());
    _objc_inform("RAW ISA:  %s%s%s", nameForLogging(), isMetaClass() ? " (meta)" : "", inherited ? " (inherited)" : "");
}
...
void objc_class::setInstancesRequireRawIsaRecursively(bool inherited) {
    ...
    foreach_realized_class_and_subclass(cls, [=](Class c) {
        ...
        if (PrintRawIsa)
            c->printInstancesRequireRawIsa(inherited || c != cls);
        return true;
    });
}
...
void objc_class::setDisallowPreoptCachesRecursively(const char *why) {
    ...
    foreach_realized_class_and_subclass(cls, [=](Class c) {
        ...
        if (c->cache.isConstantOptimizedCache(/* strict */ true)) {
            ...
            if (PrintCaches) {
                _objc_inform("CACHES: %sclass %s: disallow preopt cache (from %s)", isMetaClass() ? "meta" : "", nameForLogging(), why);
            }
...
void objc_class::setDisallowPreoptInlinedSelsRecursively(const char *why) {
    ...
    foreach_realized_class_and_subclass(cls, [=](Class c) {
        ...
        if (PrintCaches) {
            _objc_inform("CACHES: %sclass %s: disallow sel-inlined preopt cache (from %s)", isMetaClass() ? "meta" : "", nameForLogging(), why);
        }
...
void objc_class::chooseClassArrayIndex() {
    ...
    if (index == 0)
        index = objc_indexed_classes_count++;  // index 0 is unused
...
void objc_setHook_lazyClassNamer(_Nonnull objc_hook_lazyClassNamer newValue, _Nonnull objc_hook_lazyClassNamer *_Nonnull oldOutValue) {
...
const uint8_t *class_getIvarLayout(Class cls) {
    if (cls)
        return cls->data()->ro()->getIvarLayout();
    else
        return nil;
}
...
const uint8_t *class_getWeakIvarLayout(Class cls) {
    if (cls)
        return cls->data()->ro()->weakIvarLayout;
    else
        return nil;
}
 ...
 * Locking: runtimeLock must be held by the caller
 **********************************************************************/
static SEL *addMethods(Class cls, const SEL *names, const IMP *imps, const char **types, uint32_t count, bool replace, uint32_t *outFailedCount) {
    runtimeLock.assertLocked();
        ...
        if ((m = getMethodNoSuper_nolock(cls, names[i]))) {
            ...
            if (!replace) {
                ...
                if (failedNames == nil) {
                    ...
                    // every method fails
                    failedNames = (SEL *)calloc(sizeof(*failedNames), count + 1);
                }
...
SEL *class_addMethodsBulk(Class cls, const SEL *names, const IMP *imps, const char **types, uint32_t count, uint32_t *outFailedCount) {
    if (!cls) {
        if (outFailedCount)
            *outFailedCount = count;
...
void class_replaceMethodsBulk(Class cls, const SEL *names, const IMP *imps, const char **types, uint32_t count) {
    if (!cls)
        return;
...
BOOL class_addProtocol(Class cls, Protocol *protocol_gen) {
    ...
    if (!cls)
        return NO;
    if (class_conformsToProtocol(cls, protocol_gen))
        return NO;
    ...
    // fixme optimize
    protocol_list_t *protolist = (protocol_list_t *)malloc(sizeof(protocol_list_t) + sizeof(protocol_t *));
    protolist->count = 1;
...
static bool _class_addProperty(Class cls, const char *name, const objc_property_attribute_t *attrs, unsigned int count, bool replace) {
    ...
    if (prop && !replace) {
        ...
        property_list_t *proplist = (property_list_t *)malloc(property_list_t::byteSize(sizeof(property_t), 1));
        proplist->count = 1;
...
Class look_up_class(const char *name, bool includeUnconnected __attribute__((unused)), bool includeClassHandler __attribute__((unused))) {
    ...
    if (!result) {
        ...
        if (tls->classNameLookupsUsed == tls->classNameLookupsAllocated) {
            tls->classNameLookupsAllocated = (tls->classNameLookupsAllocated * 2 ?: 1);
            size_t size = tls->classNameLookupsAllocated * sizeof(tls->classNameLookups[0]);
            tls->classNameLookups = (const char **)realloc(tls->classNameLookups, size);
        }
...
Class objc_duplicateClass(Class original, const char *name, size_t extraBytes) {
    ...
    *(char **)&ro->name = strdupIfMutable(name);
    ...
    if (PrintConnecting) {
        _objc_inform("CLASS: realizing class '%s' (duplicate of %s) %p %p", name, original->nameForLogging(), (void *)duplicate, ro);
    }
...
bool verifySuperclass(Class superclass, bool rootOK) {
    ...
    if (!superclass->isRealized())
        return false;

    // Superclass must not be under construction.
    if (superclass->data()->flags & RW_CONSTRUCTING)
        return false;
...
Class objc_initializeClassPair(Class superclass, const char *name, Class cls, Class meta) {
    ...
    if (getClassExceptSomeSwift(name) || !verifySuperclass(superclass, true /*rootOK*/)) {
        return nil;
    }
...
void objc_registerClassPair(Class cls) {
    ...
    if ((cls->data()->flags & RW_CONSTRUCTED) || (cls->ISA()->data()->flags & RW_CONSTRUCTED)) {
    ...
    if (!(cls->data()->flags & RW_CONSTRUCTING) || !(cls->ISA()->data()->flags & RW_CONSTRUCTING)) {
...
Class objc_readClassPair(Class bits, const struct objc_image_info *info) {
    ...
    if (cls != bits) {
        ...
        _objc_fatal("objc_readClassPair for class %s changed %p to %p", cls->nameForLogging(), bits, cls);
    }
...
void objc_disposeClassPair(Class cls) {
    ...
    if (!(cls->data()->flags & (RW_CONSTRUCTED | RW_CONSTRUCTING)) || !(cls->ISA()->data()->flags & (RW_CONSTRUCTED | RW_CONSTRUCTING))) {
    ...
    if (cls->data()->firstSubclass) {
        _objc_inform("objc_disposeClassPair: class '%s' still has subclasses, "
                     "including '%s'!",
                     cls->data()->ro()->getName(), cls->data()->firstSubclass->nameForLogging());
    }
    if (cls->ISA()->data()->firstSubclass) {
        _objc_inform("objc_disposeClassPair: class '%s' still has subclasses, "
                     "including '%s'!",
                     cls->data()->ro()->getName(), cls->ISA()->data()->firstSubclass->nameForLogging());
    }
...
unsigned class_createInstances(Class cls, size_t extraBytes, id *results, unsigned num_requested) {
    return _class_createInstancesFromZone(cls, extraBytes, nil, results, num_requested);
}
...
static id _object_copyFromZone(id oldObj, size_t extraBytes, void *zone) {
    if (oldObj->isTaggedPointerOrNil())
        return oldObj;
    ...
    size_t size;
    id obj = _class_createInstanceFromZone(cls, extraBytes, zone, OBJECT_CONSTRUCT_NONE, false, &size);
    if (!obj)
        return nil;
...
void *objc_destructInstance(id obj) {
    if (obj) {
        ...
        if (assoc)
            _object_remove_assocations(obj, /*deallocating*/ true);

...
template <typename T> T *getDataSection(const headerType *mhdr, const char *sectname, size_t *outBytes, size_t *outCount) {
...
uint32_t *getLibobjcInitializerOffsets(const headerType *mhdr, size_t *outCount) {
    ...
    if (outCount)
        *outCount = byteCount / sizeof(uint32_t);
    return offsets;
}

...
NXHashTable *NXCopyHashTable(NXHashTable *table) {
    ...
    newt = ALLOCTABLE(z);
    newt->prototype = table->prototype;
    newt->count = 0;
...
void *NXHashGet(NXHashTable *table, const void *data) {
    ...
    if (!j)
        return NULL;
    if (j == 1) {
        return ISEQUAL(table, data, bucket->elements.one) ? (void *)bucket->elements.one : NULL;
    };
    pairs = bucket->elements.many;
    while (j--) {
        /* we don't cache isEqual because lists are short */
...
void *NXHashInsert(NXHashTable *table, const void *data) {
    ...
    if (bucket->count)
        bcopy((const char *)bucket->elements.many, (char *)(newt + 1), bucket->count * PTRSIZE);
    *newt = data;
    FREEPAIRS(bucket->elements.many);
    bucket->count++;
    bucket->elements.many = newt;
...
void *NXHashRemove(NXHashTable *table, const void *data) {
    ...
    while (j--) {
        if (ISEQUAL(table, data, *pairs)) {
            ...
            newt = (bucket->count - 1) ? ALLOCPAIRS(z, bucket->count - 1) : NULL;
            if (bucket->count - 1 != j)
...
const NXHashTablePrototype NXPtrPrototype = {NXPtrHash, NXPtrIsEqual, NXNoEffectFree, 0};

const NXHashTablePrototype NXStrPrototype = {NXStrHash, NXStrIsEqual, NXNoEffectFree, 0};

const NXHashTablePrototype NXPtrStructKeyPrototype = {hashPtrStructKey, isEqualPtrStructKey, NXReallyFree, 0};

const NXHashTablePrototype NXStrStructKeyPrototype = {hashStrStructKey, isEqualStrStructKey, NXReallyFree, 0};

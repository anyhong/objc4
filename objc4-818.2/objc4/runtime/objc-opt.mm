...
bool noMissingWeakSuperclasses(void) {
    if (!preoptimized)
        return NO;  // might have missing weak superclasses
...
Protocol *getSharedCachePreoptimizedProtocol(const char *name) {
    ...
    return (Protocol *)protocols->getProtocol(name, [](const void *hi) -> bool { return ((header_info *)hi)->isLoaded(); });
}

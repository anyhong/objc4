...
static void _finishInitializing(Class cls, Class supercls) {
    ...
    if (it == pendingInitializeMap->end())
        return;
...
static void _finishInitializingAfter(Class cls, Class supercls) {
    ...
    if (PrintInitializing) {
        _objc_inform("INITIALIZE: thread %p: class %s will be marked as fully "
                     ...
                     objc_thread_self(), cls->nameForLogging(), supercls->nameForLogging());
    }
...
BREAKPOINT_FUNCTION(void objc_initializeAfterForkError(Class cls));
...
void performForkChildInitialize(Class cls, Class supercls) {
    if (classHasTrivialInitialize(cls)) {
        ...
        _objc_inform_now_and_on_crash("+[%s initialize] may have been in progress in another thread "
                                      "when fork() was called.",
                                      cls->nameForLogging());
        objc_initializeAfterForkError(cls);
        _objc_fatal("+[%s initialize] may have been in progress in another thread "
...
void initializeNonMetaClass(Class cls) {
    ...
    if (reallyInitialize) {
        ...
        if (PrintInitializing) {
            _objc_inform("INITIALIZE: thread %p: calling +[%s initialize]", objc_thread_self(), cls->nameForLogging());
        }

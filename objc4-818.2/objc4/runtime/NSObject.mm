...
static void callSetWeaklyReferenced(id obj) {
    ...
    if (slowpath(cls->hasCustomRR() && !object_isClass(obj))) {
        ...
        void (*setWeaklyReferenced)(id, SEL) = (void (*)(id, SEL))class_getMethodImplementation(cls, @selector(_setWeaklyReferenced));
...
template <HaveOld haveOld, HaveNew haveNew, enum CrashIfDeallocating crashIfDeallocating> static id storeWeak(id *location, objc_object *newObj) {
...
#define POOL_BOUNDARY nil
...
#if TARGET_OS_IPHONE && !TARGET_OS_SIMULATOR
        bool objcModeNoFaults = DisableFaults || getpid() == 1 || !os_variant_has_internal_diagnostics("com.apple.obj-c");
        if (!objcModeNoFaults) {
    ...
    AutoreleasePoolPage(AutoreleasePoolPage *newParent) : AutoreleasePoolPageData(begin(), objc_thread_self(), newParent, newParent ? 1 + newParent->depth : 0, newParent ? newParent->hiwat : 0) {
    ...
    template <typename Fn> void busted(Fn log) const {
        magic_t right;
        log("autorelease pool page %p corrupted\n"
            ...
            "  pthread   %p\n"
            "  should be %p\n",
            this, magic.m[0], magic.m[1], magic.m[2], magic.m[3], right.m[0], right.m[1], right.m[2], right.m[3], this->thread, objc_thread_self());
    }

    __attribute__((noinline, cold, noreturn)) void busted_die() const {
        busted(_objc_fatal);
        __builtin_unreachable();
    }

    inline void check(bool die = true) const {
    ...
    static void tls_dealloc(void *p) {
        ...
        if (AutoreleasePoolPage *page = coldPage()) {
            if (!page->empty())
                objc_autoreleasePoolPop(page->begin());  // pop all of the pools
...
public:
    ...
    __attribute__((noinline, cold)) static void popPageDebug(void *token, AutoreleasePoolPage *page, id *stop) {
    ...
    __attribute__((noinline, cold)) void print() {
        ...
        for (id *p = begin(); p < next; p++) {
            if (*p == POOL_BOUNDARY) {
                ...
                if (entry->count > 0) {
                    id obj = (id)entry->ptr;
                    _objc_inform("[%p]  %#16lx  %s  autorelease count %u", p, (unsigned long)obj, object_getClassName(obj), entry->count + 1);
                    goto done;
                }
#endif
...
bool objc_object::sidetable_present() {
    ...
    if (weak_is_registered_no_lock(&table.weak_table, (id)this))
        result = true;
...
void objc_object::sidetable_clearExtraRC_nolock() {
    ASSERT(isa.nonpointer);
    SideTable &table = SideTables()[this];
    RefcountMap::iterator it = table.refcnts.find(this);
...
__attribute__((aligned(16), flatten, noinline)) id objc_retain(id obj) {
    if (obj->isTaggedPointerOrNil())
        return obj;
    return obj->retain();
}
...
__attribute__((aligned(16), flatten, noinline)) id objc_autorelease(id obj) {
    if (obj->isTaggedPointerOrNil())
        return obj;
...
NEVER_INLINE id _objc_rootRetain(id obj) {
    ASSERT(obj);
...
id objc_retainAutoreleaseReturnValue(id obj) {
    if (prepareOptimizedReturn(ReturnAtPlus0))
        return obj;
...
id objc_unsafeClaimAutoreleasedReturnValue(id obj) {
    if (acceptOptimizedReturn() == ReturnAtPlus0)
        return obj;
...
id objc_retainedObject(objc_objectptr_t pointer) {
    return (id)pointer;
}

// convert objc_objectptr_t to id, without ownership transfer.
id objc_unretainedObject(objc_objectptr_t pointer) {
    return (id)pointer;
}

// convert id to objc_objectptr_t, no ownership transfer.
objc_objectptr_t objc_unretainedPointer(id object) {
    return object;
}
...
__attribute__((objc_nonlazy_class)) @implementation __NSUnrecognizedTaggedPointer
- (id)retain {
    return self;
}
- (oneway void)release {
...
__attribute__((objc_nonlazy_class)) @implementation NSObject

...
template <class T> T *Zone<T, false>::alloc_slow() {
    ...
    _freelist.push_list(reinterpret_cast<void *>(&slab[1]), reinterpret_cast<void *>(&slab[n_elem - 1]));
...
#if __OBJC2__
#define ZoneInstantiate(type) template class Zone<type, sizeof(type) % MALLOC_ALIGNMENT == 0>
